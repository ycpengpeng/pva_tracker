...
// Coefficientsros::Publisher setpoint_pub
...
void velocityCallback(const geometry_msgs::TwistStamped::ConstPtr &msg)
{
    ...
    mavros_msgs::PositionTarget lp;
    lp.header.stamp = ros::Time::now();

    lp.type_mask=1+2+4+8+16+32+64+128+256+512;

    lp.position.x = current_p(0);
    lp.position.y = current_p(1);
    lp.position.z =current_p(2);

    lp.velocity.x=current_v(0);
    lp.velocity.y=current_v(1);
    lp.velocity.z=current_v(2);

	localpoint_pub.publish(lp)




    //ROS_INFO("current_v: %f  %f  %f ", current_v(0),current_v(1),current_v(2));
...
void pvaCallback(const trajectory_msgs::JointTrajectoryPoint::ConstPtr& msg)
{
    ...
    mavros_msgs::GlobalPositionTarget sp;


    sp.header.stamp = ros::Time::now();

    sp.type_mask=1+2+4+8+16+32+64+128+256+512;
    sp.acceleration_or_force.x=planned_p(0);
    sp.acceleration_or_force.y=planned_p(1);

    sp.acceleration_or_force.z=planned_p(2);

    sp.velocity.x=planned_v(0);
    sp.velocity.y=planned_v(1);
    sp.velocity.z=planned_v(2);
    ...
    {
        p_error_accumulate=Vector3d::Zero();
        v_error_accumulate=Vector3d::Zero();
    }
    ...
    ROS_INFO_THROTTLE(1.0, "Attitude Quaternion Setpoint is w=%f, x=%f, y=%f, z=%f, thrust=%f", att_des_q.w(),   att_des_q.x(), att_des_q.y(), att_des_q.z(), thrust_des);
...
void tracker_update_Callback(const pva_tracker_pp::input::ConstPtr& msg)
{
    ...
    //ROS_INFO_THROTTLE(2,"predict: %f  %f   %f  %f  %f  %f    ",predict[0],predict[1],predict[2],predict[3],predict[4],predict[5]);
    ...
    //ROS_INFO_THROTTLE(1,"predict: %f  %f   %f      ",a_des(0),a_des(1),a_des(2));
...
//    ROS_INFO("after compute roll: %f pitch:  %f yaw: %f  ",euler_angle(0),euler_angle(1),euler_angle(2));
    ...
    ROS_INFO_THROTTLE(1.0, "Attitude Quaternion Setpoint update------- is w=%f, x=%f, y=%f, z=%f, thrust=%f", att_setpoint.orientation.w,
                      att_setpoint.orientation.x, att_setpoint.orientation.y, att_setpoint.orientation.z, att_setpoint.thrust);
...
void configureCallback(tracker::PVA_TrackerConfig &config, uint32_t level) {
    ...
    ROS_INFO("position_p_xy: %f",config.position_p_xy);
...
int main(int argc, char** argv) {
    ros::init(argc, argv, "mpc_tracker_pytorch_lss");
    ...
    //ros::Subscriber position_sub = nh.subscribe("/vicon_pose", 1, positionCallback);

    ros::Subscriber pose_sub = nh.subscribe<geometry_msgs::PoseStamped>("/mocap/pose_01", 1, positionCallback);

   //ros::Subscriber velocity_sub = nh.subscribe("/vicon_vel", 1, velocityCallback);

    ros::Subscriber velocity_sub = nh.subscribe("/mocap/vel_01", 1, velocityCallback);
    ...
    setpoint_pub=nh.advertise<mavros_msgs::GlobalPositionTarget>("mavros/setpoint_raw/global",1);


    ros::Subscriber state_sub = nh.subscribe<mavros_msgs::State>("/mavros/state", 1, stateCallback);

    localpoint_pub = nh.advertise<mavros_msgs::PositionTarget>("mavros/setpoint_raw/local", 1);
