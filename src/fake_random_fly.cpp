...
ROS_INFO("AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA");

    Matrix<double,number_of_points,3> point_raw_matrix;
    float side=4.0;
    float fly_height=10;
    point_raw_matrix<<0 , 0,0,
		    3.0, 0,0,
		     5.5, -3.0,0,
		    1.2,-6.5,0,
		    0 ,-1.8,0
            		;
    Matrix<double,number_of_points,3> point_raw_velocity_matrix;

    float tmp_vel=3.0; ////////////////////////////////////
    point_raw_velocity_matrix<<tmp_vel , 0,0,
                                tmp_vel, 0,0,
                            0 , -tmp_vel,0,
                                -tmp_vel,0,0,
                                -2.0,0,0
    ...
    Matrix<double,3,number_of_points>  point_velocity_matrix;

    point_velocity_matrix = rotation_matrix*point_raw_velocity_matrix.transpose();

    for(int i=0;i<=number_of_points-1;i++)
    {
        point_matrix(2,i)+=take_off_height;
    ...
    Vector3d last_velocity_point= point_velocity_matrix.col(0);

    motion_primitives(last_point, last_velocity_point, a0, point_matrix.col(point_number), point_velocity_matrix.col(point_number), af, 1.0, delt_t, p_t, v_t, a_t, t_vector);  ///////////////////////////////
    int i=0;
    ...
    while(ros::ok())
    {
        ...
        if(i>=t_vector.size()-1)
        {
			...
			last_velocity_point=point_velocity_matrix.col(point_number);
		    point_number++;
			...
			motion_primitives(last_point,last_velocity_point, a0, point_matrix.col(point_number),  point_velocity_matrix.col(point_number), af, 2.4, delt_t, p_t, v_t, a_t, t_vector); //////////////////////////
			i=0;
        }
